<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@page import="java.sql.*, in.tourstravels.connection.connector, in.tourstravels.dto.booking"%>
...
<%
    ...
    int bookingId = -1;
    booking bookingDetails = null; // DTO to hold booking data
    ...
    try {
        if (bookingIdStr != null && !bookingIdStr.isEmpty()) {
            ...
            if (rs.next()) {
                ...
                bookingDetails.setBooking_id(rs.getInt("booking_id"));
                ...
                bookingDetails.setPlaces_visit(rs.getString("places_visit"));
                bookingDetails.setPackage_img(rs.getString("package_img")); // Although not used in PDF, good to retrieve
                bookingDetails.setNum_person(rs.getInt("num_person"));
                bookingDetails.setTotal_price(rs.getDouble("total_price"));
                bookingDetails.setPay_mode(rs.getString("pay_mode"));
                bookingDetails.setStatus(rs.getString("status"));
                bookingDetails.setNo_of_days(rs.getInt("no_of_days"));
                bookingDetails.setFrom_date(rs.getDate("from_date"));
                bookingDetails.setTo_date(rs.getDate("to_date"));
                bookingDetails.setBooked_date(rs.getTimestamp("booked_date"));
            }
        }
    ...
    <div id="pdfContent" class="pdf-container">
        ...
        <table border=2 class="booking-details-table">
            <tr>
                ...
                <td><%= bookingDetails.getBooking_id() %></td>
            </tr>
            <tr>
            ...
            <tr>
                <td style="text-align:left;">Places to Visit:</td>
                <td><%= bookingDetails.getPlaces_visit() %></td>
            </tr>
            <tr>
                <td style="text-align:left;">Number of Persons:</td>
                <td><%= bookingDetails.getNum_person() %></td>
            </tr>
            <tr>
                <td style="text-align:left;">Total Price:</td>
                <td>₹ <%= String.format("%.2f", bookingDetails.getTotal_price()) %></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td><%= bookingDetails.getNo_of_days() %></td>
            </tr>
            <tr>
                <td style="text-align:left;">From Date:</td>
                <td><%= dateFormat.format(bookingDetails.getFrom_date()) %></td>
            </tr>
            <tr>
                <td style="text-align:left;">To Date:</td>
                <td><%= dateFormat.format(bookingDetails.getTo_date()) %></td>
            </tr>
            <tr>
                <td style="text-align:left;">Booked Date:</td>
                <td><%= dateTimeFormat.format(bookingDetails.getBooked_date()) %></td>
            </tr>
        </table>
        ...
        window.onload = function () {
            document.getElementById("downloadPdf")
                .addEventListener("click", () => {
                    ...
                    var opt = {
                        margin: 0.5,
                        filename: 'bookingConfirmation_<%= bookingDetails.getBooking_id() %>.pdf',
