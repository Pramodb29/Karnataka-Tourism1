...
<%@page import="in.tourstravels.dto.places"%>
<%@page import="in.tourstravels.dao.PlacesDAO"%>
<%@page import="in.tourstravels.dao.PlacesDAOImpl"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<%
    ...
    ArrayList<places> al = pdao.getallPlacesinfo();

    if (al != null && !al.isEmpty()) {
        for (places s : al) {
%>
    <div class="product-card">
        <img src="images/<%= s.getImage() %>" />
        <div class="content-wrapper">
            <ul class="details-list">
            <li><b>Package name:</b> <%= s.getPackage_name() %></li> 
                <li><b>Places Visit:</b> <%= s.getSel_places() %></li>                        
                <li><b>Description:</b> <%= s.getDescription() %></li>
                 <li><b>Duration(In Days):</b> <%= s.getDuration() %></li>
            </ul>
            
            
            <div class="buttons1">
             <form action="updateplace.jsp?pid=<%= s.getPlace_id() %>" method="post">
                    <input type="hidden" name="pid" value="<%= s.getPlace_id() %>" />
                    <button type="submit" class="btn" style="background-color:blue;color:white;width:100%;">Update</button>
                </form>
                
                  <form action="addPlaceAction" method="post" onsubmit="return confirm('Are you sure you want to delete this package?');">
                    <input type="hidden" name="action" value="delete" /> 
                    <input type="hidden" name="p_id" value="<%= s.getPlace_id() %>" />
