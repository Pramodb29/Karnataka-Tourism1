...
<%
    ...
    ArrayList<guide> al = gdao.getallGuidesinfo();

    if (al != null && !al.isEmpty()) {
        for (guide g : al) {
%>
    <div class="product-card">
        <img src="images4/<%= g.getGuide_img() %>" />
        <div class="content-wrapper">
            <ul class="details-list">
                <li><b>Guide Name:</b> <%= g.getGuide_name() %></li>                        
                <li><b>Location:</b> <%= g.getLocation() %></li>
                <li><b>Languages:</b> <%= g.getLanguages() %></li>
                 <li><b>Cost per Day: </b> <%= g.getDaily_charge() %></li>
                  <li><b>Phone No: </b> <%= g.getPhone_no() %></li>
                  
            </ul>
            
            
            <div class="buttons1">
            <form action="updateGuide.jsp?pid=<%= g.getGuide_id() %>" method="post">
                    <input type="hidden" name="pid" value="<%= g.getGuide_id() %>" />
                    <button type="submit" class="btn" style="background-color:blue;color:white;width:100%;">Update</button>
                </form>
                
                  <form action="addGuideAction" method="post" onsubmit="return confirm('Are you sure you want to delete this vehicle?');">
                    <input type="hidden" name="action" value="delete" /> 
                    <input type="hidden" name="p_id" value="<%= g.getGuide_id() %>" />
